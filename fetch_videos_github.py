...
# GitHub Actions 里环境变量直接从 Secrets 传入
YOUTUBE_API_KEY = os.environ.get("YOUTUBE_API_KEY")
APIYI_TOKEN = os.environ.get("OPENAI_API_KEY")
COOKIES_CONTENT = os.environ.get("YOUTUBE_COOKIES")  # 新增：从 Secrets 读取 cookies
...
def get_transcript(video_id):
    """使用 yt-dlp 获取字幕（从环境变量读取 cookies）"""
    url = f"https://www.youtube.com/watch?v={video_id}"
    
    # 如果有 cookies 内容，先创建临时文件
    cookies_file = None
    if COOKIES_CONTENT:
